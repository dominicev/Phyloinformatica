...
versionNo=0.92;
Print["Biosystematica version "<>ToString[versionNo]]
...
(* ::Text:: *)
(*FUNCTIONALITY: Biosystematica was developed to provide basic phylogenetics/bioinformatics workflow functionality that can be easily extended to "genomic" scales (e.g. hundreds of thousands of files or very large files). Most of the things you can do in Biosystematica are basic things you may have done in Mesquite, or another alignment manipulation software, when dealing with a small (~1-10 genes, 3-250 individuals/taxa) dataset. In this sense, the functionality of Biosystematica greatly overlaps with that of Biopython and Biopython actually has greater functionality. The advantage of using Biosystematica over Biopython are mostly Mathematica specific (i.e. if you prefer the Mathematica interface or programming language). However, there is some other functionality provided by Biosystematica and not in Biopython (e.g. some tree-file operations, alignment statistics, nucleotide translation, RADICAL workflow). Most importantly,  Biosystematica is designed to be user-friendly and requires only a basic understanding of coding and the Mathematica language to use.*)
(**)
...
(*4. While much of Mathematica is built-in there are some user made packages and code (such as Biosystematica!). Google them.*)
...
(* ::Subsection::Closed:: *)
(*Matching PartitionFinder schemes with a Biosystematica generated RAXML_part file*)
