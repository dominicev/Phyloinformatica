...
versionNo=0.93;
Print["Phyloinformatica version "<>ToString[versionNo]]
...
(* ::Input:: *)
(*"Phyloinformatica version "<>ToString[versionNo]<>" created on "<>DateString[]<>recentChanges;*)


(* ::Text:: *)
(*"Phyloinformatica version 0.93 created on Sun 20 Jan 2019 17:56:23\nChanges:\n-Name changed from 'Biosystematica' to 'Phyloinformatica'\n";*)
...
(* ::Text:: *)
(*Update version number and add changes to "recentChanges" object. Run changelog code and save as a text cell.*)
...
(**)
(*START WORKING: If you are new to Mathematica and you don't know how to run the functions in this notebook see the section below. You should be able to understand most of the "Backend" if you have an intermediate understanding of Mathematica. Otherwise, skip to the "Basic Phyloinformatica primer" section for some basic usage notes.*)
...
(* ::Text:: *)
(*You probably noticed by now that Mathematica code is written in Mathematica formatted notebooks. The formatting utilizes nested sections that you can expand or contract depending upon your focus. You can expand or contract by double clicking the relevant bracket in the right margin. Play around with it to see how it works. Create new cells by clicking between existing cells and simply typing (this will create a cell of the default type) or by clicking between cells and pressing "Alt" and a number from 1 to 9. Each number creates a different kind of cell. You Can created nested cells in the way.*)


(* ::Text:: *)
(*To STOP executed code from finishing you can press "Alt-." (Alt Period) or you can goto the Evaluation tab on the taskbar and click: Quit Kernel->Local. *)
(**)
...
(* ::Text:: *)
(*The function "table" has not been defined, which is why it appears in blue and does not give any interesting output when you run it.*)
...
(* ::Text:: *)
(*Above you can see an example of the syntax for lists and matrices. The second example given happens to use the matrix structure that comprises the fastaParsed objects used by Phyloinformatica.*)
...
(*2. Mathematica uses lots of intuitive input types, so if you are creative you can find interesting ways to run code. For example, for doing image transformations you can run functions on images stored as objects, or you can actually paste the image into the code itself and run it that way. Really cool.*)
...
(*4. While much of Mathematica is built-in there are some user made packages and code (such as Phyloinformatica!). Google them.*)
(*5. Map is faster than Table. ParallelMap and ParallelTable can be faster than the base versions but not always, and sometimes they give incorrect output (e.g. if the order of the loop iterations matters), so be careful.*)
...
(*7. Dynamic and Manipulate are awesome functions, but can crash notebooks. Use them but be careful.*)


(* ::Subsection::Closed:: *)
(*Basic Phyloinformatica primer*)


(* ::Text:: *)
(*The functions in this package are biased towards my own work flows. For instance, since I prefer working with FASTA files and most of the softwares I use take FASTA files as input, this is the filetype that the package works with most (although everything works just as well with Phylip files). Many output formats and parsing functions are designed for usage in RAXML, MAFFT, and IQTree but there are exceptions.*)
...
(* ::Text:: *)
(*----------------------------------------------------------------------*)
(*--Themes and patterns among Phyloinformatica functions--*)
(*----------------------------------------------------------------------*)


(* ::Text:: *)
(*Functions in Phyloinformatica are designed to have flexible input whenever possible, but with some limitations. *)
(**)
(*Most functions dealing with sequences or alignments can take files (as fasta or phylip) or fastaParsed objects as input. Functions usually detect the input automatically. In some cases, the same goes for individual files or lists of files: the functions automatically execute appropriately depending upon either case (although as of v. 0.92 this functionality is fairly limited).*)
(**)
(*When lists of files or lists of fastaParsed objects are not allowed as input, the Mathematica base function Map (or it's shorthand equivalent: f[#]&/@list) needs to be used to extend its functionality. If you require this, learn how to use Mathematica if you don't already (see previous section).*)
(**)
...
(**)
(*When file names are provided as input the output is exported to the working directory as an automatically renamed output file. However, the option "Hold" can be added to the function's input to specify that a fastaParsed object should be returned instead of exporting to file. If you want to do multiple operations on a file sequentially, it is best to use the "Hold" option or work only with fastaParsedObjects. However, my personal preference is to export to file and then check manually that the script did what you wanted it to  (my preferred alignment viewer for this purpose is AliView). If you aren't using the "Hold" option and you want to do multiple operations on files, be careful about managing the input and output file names and that you aren't overwriting the wrong file at the wrong time.*)
...
(* ::Text:: *)
(*One of the reasons I started Phyloinformatica was the limited ability of some softwares to deal with "big data" sequence files. Mathematica does a pretty good job of not crashing under the load of 1GB (or more) of memory loaded onto the kernel (at least on my machine). However, that doesn't mean it's fast when it's doing it. Hopefully, I will update more functions to utilize low-level stream-reading functionality, which provides big gains in terms of speed and memory (although it's more difficult to code). *)
...
sequenceExtract[file_, header_String, "Hold"];
...
(*  	*)
(*  Output is a list of file names with the number of stop codons in the alignment for three possible reading frames.*)
...
(* ::Subsubsection::Closed:: *)
(*Comparing two aligned sequences*)


(* ::Text:: *)
(*The function below extracts two sequences from am alignment and counts the number of non-overlapping regions / indels (when there is an ambiguous character in one sequence and a non-ambiguous character in another sequence) and the number of point differences (e.g. a mutational difference in the nucleotide/amino acid sequence).*)
(**)
(*Input:*)
(*	locus - a fasta file, or list of fasta files containing an alignment*)
(*	taxon1 - a string with the exact name of a taxon or other part of a sequence header in the alignment. If this name appears more than once in the alignement the script will not work.*)
(*	taxon2 - a string with the exact name of a taxon or other part of a sequence header in the alignment. If this name appears more than once in the alignement the script will not work.*)
(*	ambiguousCharacters - a list of single character strings representing the gap characters and ambiguous characters in the alignment. Default: treat the alignment as a nucleotide alignment and use the list {"-", "?", "N","n"}.*)


compareTwoAlignedSequences[locus_String, taxon1_String, taxon2_String,ambiguousCharacters_List]


compareTwoAlignedSequences[locus_List, taxon1_String, taxon2_String,ambiguousCharacters_List]
...
(* ::Text:: *)
(*The tree comparisons can be done two ways: 1. by the Robinson-Fould's metric, 2. Consensus Fork Index, 3. Mean bootstrap of a summary tree. Since IQTree can calculate 1, it is the simplest option. IQTree+Phyloinformatica can calculate 3. The function currently only uses 1 but I can easily update it to do 3 or potentially 2. *)
...
(* ::Subsection::Closed:: *)
(*Matching PartitionFinder schemes with a Phyloinformatica generated RAXML_part file*)


(* ::Text:: *)
(*Anytime you concatenate an alignment in Phyloinformatica, you also output a RAXML style partitions file. You can use this to specify blocks in partitionFinder. partitionFinder then lumps them based on its analysis and you can use their output to partition your analysis.*)
(**)
...
(**)
(*In order for this to work, you need to have the output of the Phyloinformatica concatenation and the pFinder "best_scheme.txt" output in the working directory. Just invoking the function with no options will run the script assuming the default file names. Specifying the input, you can put renamed files.*)
(**)
(*Input is:*)
...
importAlignment[directory_String, "ImportTogether"]:=Block[{fn},
...
sequenceExtract[fasta_String, header_String, "Hold"]:=Block[{ali},
ali[[#]]&/@Position[ali=importAlignment[fasta], header][[All, 1]]
]


sequenceExtract[fasta_String, header_String]:=Block[{ali},
fastaExport[header<>"_"<>fasta,ali[[#]]&/@Position[ali=importAlignment[fasta], header][[All, 1]]]
]


sequenceExtract[fasta_List, header_String]:=Block[{ali},
ali[[#]]&/@Position[ali=fasta, header][[All, 1]]
]


(* ::Text:: *)
(*Below are some obsolete versions that weren't working well*)
...
{Append[#[[1]], "RevComp"], reverseComplementDNA[#[[2]]]}&/@fastaParsed
]


reverseComplementAll[fastaParsed_String, "Hold"]:=reverseComplementAll[Evaluate[fastaParsed//importAlignment]]
...
b=(StringPartition[StringDrop[sequence//ToUpperCase//reverseComplementDNA,#], 3]&/@{0, 1, 2});
...
concatenateAlignmentsFromFile[alignmentFiles_List,locusNamePosition_,taxonNamePosition_,dataType_String]:=Block[{out, alignments},
alignments=SortBy[importAlignment[#], #[[1, taxonNamePosition]]&]&/@alignmentFiles;
Export["concatAlign_"<>(Riffle[ToString/@Date[], "-"]//StringJoin)<>".fasta",out=concatAlign[alignments], "Text"];
...
(countStopCodons[importAlignment[fileName]])
]
...
Total/@((countStopCodonInSequence[#]&/@fastaParsed[[All, 2]]
)//Transpose)]
...
(*	taxon1 - a string with the exact name of a taxon or other part of a sequence header in the alignment. If this name appears more than once in the alignement the script will not work.*)
(*	taxon2 - a string with the exact name of a taxon or other part of a sequence header in the alignment. If this name appears more than once in the alignement the script will not work.*)
(*	ambiguousCharacters - a list of single character strings representing the gap characters and ambiguous characters in the alignment. Default is to treat the alignment as a nucleotide alignment and use the list {"-", "?", "N","n"}.*)


ambiguousCharacters={"-", "?", "N","n"};


compareTwoAlignedSequences[locus_String, taxon1_String, taxon2_String,ambiguousCharacters_List]:=Block[{twoAlignedSequences,positionComparisons,sequenceDifferences,differences},
(
twoAlignedSequences={sequenceExtract[locus,taxon1, "Hold"][[1]],
sequenceExtract[locus,taxon2, "Hold"][[1]]};

positionComparisons=(StringPartition[ToUpperCase[#],1]&/@twoAlignedSequences[[All, 2]])//Transpose;

sequenceDifferences=DeleteCases[If[#[[1]]===#[[2]],Null, #]&/@positionComparisons, Null];

differences=DeleteCases[
Which[ContainsOnly[#, ambiguousCharacters], Null (*this is the case when the difference is actually just a difference in the ambiguous character used*),
ContainsAny[#,ambiguousCharacters ], "In/Del?",
True, "Point"
]&/@sequenceDifferences
, Null];
{{locus,Count[differences,"In/Del?"] ,Count[differences,"Point"],Union[getSequenceLengths[twoAlignedSequences]] }//Flatten}
)
]


compareTwoAlignedSequences[locus_List, taxon1_String, taxon2_String,ambiguousCharacters_List]:=Block[{},
Prepend[
Flatten[compareTwoAlignedSequences[#, taxon1, taxon2,ambiguousCharacters]&/@locus, 1]
, {"Sequence", "# of non-overlapping sites or Indels", "# of point differences", "Alignment Length"}]
]
...
SetDirectory["F:\\Dropbox\\Projects\\Phylogeny of Blaberoidea\\Alignments\\merged alignment work jan 2018\\best 100 corrected and with taxon sampling considered\\codon_partitioning\\combined\\"];


(* ::Input:: *)
(*(*nameSwap["FullresultSumtrees.tre","F:\\Dropbox\\Projects\\Phylogeny of Blaberoidea\\Alignments\\Official_taxonNameList.csv" ]*)*)
...
allSupports=ToExpression[#]&/@(StringSplit[#, {",","]"}][[1]]&/@Drop[StringSplit[tree=Import[treeName, "String"], "support="], 1]);
numTaxa=ToExpression[StringSplit[Drop[StringSplit[tree, {"ntax=", "NTAX="}], 1], ";"][[1, 1]]];
supportValues=Drop[allSupports//Sort, -numTaxa];
...
bootstrapsSummary[treeName_String, "IC"]:=Block[{(*bootTreeChim, *) ICScores, tree},

ICScores=(ToExpression/@StringSplit[StringSplit[#, "]"][[1]], ","
] &/@Drop[StringSplit[tree=Import[treeName, "String"], {"["}], 1]);


{{
{"Mean IC-value",ICScores[[All, 1]]//Mean},
{"Median IC-value",ICScores[[All, 1]]//Median},
{"Std.Dev. ICvalues",ICScores[[All, 1]]//StandardDeviation}, 
{"Distributution without 100% support",Histogram[DeleteCases[ICScores[[All, 1]]//Flatten, 1.]]},
{"Complete distribution",Histogram[ICScores[[All, 1]]//Flatten]}
}//TableForm,
{
{"Mean ICA-value",ICScores[[All, 2]]//Mean},
{"Median ICA-value",ICScores[[All, 2]]//Median},
{"Std.Dev. ICA-values",ICScores[[All, 2]]//StandardDeviation}, 
{"Distributution without 100% support",Histogram[DeleteCases[ICScores[[All, 2]]//Flatten, 1.]]},
{"Complete distribution",Histogram[ICScores[[All, 2]]//Flatten]}
}//TableForm}]
...
,Frame->False
,DataRange->{{-1, 1}, {0, 1}}
, ColorRules->(colorR={"NA"->CMYKColor[0, .5, 0, 0], Table[x->GrayLevel[1-Abs[x]], {x, -1, 0, .001}]}//Flatten)
...
,Frame->False
,DataRange->{{-1, 1}, {0, 1}}
, ColorRules->(colorR={"NA"->CMYKColor[0, .5, 0, 0], Table[(x//N)->GrayLevel[1-Abs[x]], {x, -1, 0, .001}]}//Flatten)
(*, ColorFunction\[Rule]"TemperatureMap"*)
,ColorFunction->colorFunction
,ColorFunctionScaling->True
]}&/@matrix)//TableForm
...
(colorR={"NA"->CMYKColor[0, .5, 0, 0], Table[(x//N)->GrayLevel[1-Abs[x]], {x, -1, 0, .001}]}//Flatten);
...
(* ::Subsection::Closed:: *)
(*Matching partitionFinder schemes with a Phyloinformatica RAXML_part file*)


(* ::Text:: *)
(*Anytime you concatenate an alignment in Phyloinformatica, you also output a RAXML style partitions file. You can use this to specify blocks in partitionFinder. partitionFinder then lumps them based on it's analysis and you can use their output to partition your analysis.*)
(**)
(*However, when you have a very large alignment, with many partitions, and then you edit the original alignments, your partition file no longer matches the lengths of the actual partititions. This function fixes that.*)
(**)
(*In order for this to work, you need to have the output of the Phyloinformatica concatenation and the pFinder "best_scheme.txt" output in the working directory. Just invoking the function with no options will run the script assuming the default file names. Specifying the input, you can put renamed files.*)
